...
export default function Dashboard() {
  ...
  useEffect(() => {
    supabase.auth.getSession().then(({ data: { session } }) => {
      // If no session, kick them back to the login page (not the landing page)
      if (!session) {
        router.push("/login");
      } else {
        setLoading(false);
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center p-4 md:p-6 font-sans">
      <div className="max-w-4xl w-full text-center">
        ...
        <p className="text-gray-500 mb-8 md:mb-12 text-base md:text-lg">
          Select how you want the AI to assist you today.
        </p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
          ...
          <button
            ...
            className="group md:col-span-2 relative bg-white p-8 rounded-3xl shadow-sm hover:shadow-xl border-2 border-transparent hover:border-purple-500 transition-all text-left flex flex-col md:flex-row items-center md:items-start gap-6"
          >
            ...
            <div>
              ...
              <p className="text-gray-500 leading-relaxed">
                Practice mode. The AI asks you questions, listens to your
                answers, and gives you a grade and feedback at the end. Perfect
                for warm-ups.
              </p>
            </div>
          </button>

          {/* OPTION 4: RESUME OPTIMIZER */}
          <button
            onClick={() => router.push("/optimizer")}
            className="group md:col-span-2 relative bg-white p-8 rounded-3xl shadow-sm hover:shadow-xl border-2 border-transparent hover:border-orange-500 transition-all text-left flex flex-col md:flex-row items-center md:items-start gap-6"
          >
            <div className="absolute top-0 right-0 bg-orange-500 text-white text-xs font-bold px-3 py-1 rounded-bl-xl rounded-tr-2xl shadow-sm">
              NEW FEATURE
            </div>
            <div className="bg-orange-100 w-16 h-16 rounded-2xl flex items-center justify-center group-hover:scale-110 transition-transform flex-shrink-0">
              <span className="text-3xl">📄</span>
            </div>
            <div>
              <h2 className="text-2xl font-bold text-gray-900 mb-2">
              ...
              <p className="text-gray-500 leading-relaxed">
                Beat the ATS. Paste your resume and the target job description,
                and our AI will rewrite your bullets to perfectly match what the
        ...
        <button
          ...
          className="mt-8 md:mt-12 mb-8 text-gray-400 hover:text-gray-600 underline text-sm font-medium"
        >
          Log Out
        </button>
      </div>
